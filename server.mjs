...
const DATA_DIR = path.resolve(process.env.SEVENDTD_DATA_DIR || process.env.ICARUS_DATA_DIR || path.join(ROOT, "data"));
const STATE_FILE = path.join(DATA_DIR, "state.json");
const LEGACY_CONFIG = path.join(ROOT, "config.json");
const PORT = Number(process.env.SEVENDTD_PORT || process.env.PORT || 3240);
const HOST = process.env.SEVENDTD_HOST || process.env.ICARUS_HOST || "0.0.0.0";
...
const EXE_NAME = "7DaysToDieServer.exe";
const EXE_NAMES = ["7DaysToDieServer.exe", "7DaysToDie.exe"];
const CONFIG_FILE_NAME = "manager-serverconfig.xml";
...
function makeServer(partial = {}) {
  return {
    ...
    steamBranch: String(partial.steamBranch || partial.versionBeta || "").trim(),
    dtd: makeDtd(partial.dtd || partial.icarus || {})
  };
}

function makeDtd(partial = {}) {
  const xml = normalizeXml({
    ...defaultXml(),
    ...(partial.xml || {}),
    ServerName: partial.xml?.ServerName ?? partial.ServerName ?? partial.serverName,
    ServerPort: partial.xml?.ServerPort ?? partial.gamePort ?? partial.ServerPort,
    ServerPassword: partial.xml?.ServerPassword ?? partial.joinPassword ?? partial.ServerPassword,
    ServerMaxPlayerCount: partial.xml?.ServerMaxPlayerCount ?? partial.maxPlayers ?? partial.ServerMaxPlayerCount,
    TelnetPassword: partial.xml?.TelnetPassword ?? partial.adminPassword ?? partial.TelnetPassword,
    TelnetPort: partial.xml?.TelnetPort ?? partial.TelnetPort,
    WebDashboardPort: partial.xml?.WebDashboardPort ?? partial.WebDashboardPort,
    GameWorld: partial.xml?.GameWorld ?? partial.GameWorld,
    GameName: partial.xml?.GameName ?? partial.GameName,
    SandboxCode: partial.xml?.SandboxCode ?? partial.SandboxCode
  });
  const decoded = decodeSandboxCode(xml.SandboxCode);
  const sandbox = { ...decoded.values, ...(partial.sandbox || {}) };
  if (partial.XPMultiplier !== undefined) sandbox.XPMultiplier = Number(partial.XPMultiplier);
  if (partial.LootRespawnDays !== undefined) sandbox.LootRespawnDays = Number(partial.LootRespawnDays);
  if (partial.TraderResetInterval !== undefined) sandbox.TraderResetInterval = Number(partial.TraderResetInterval);
  xml.SandboxCode = encodeSandboxCode(sandbox, { unknown: decoded.unknown });
  syncLegacyFromSandbox(xml, sandbox);
  return {
    xml,
    sandbox,
    configFile: String(partial.configFile || CONFIG_FILE_NAME)
  };
}

function syncLegacyFromSandbox(xml, sandbox) {
  if (sandbox.XPMultiplier !== undefined) xml.XPMultiplier = Math.round(Number(sandbox.XPMultiplier) * 100);
  if (sandbox.LootRespawnDays !== undefined) xml.LootRespawnDays = Number(sandbox.LootRespawnDays);
  if (sandbox.GlobalLootCount !== undefined) xml.LootAbundance = Math.round(Number(sandbox.GlobalLootCount) * 100);
  if (sandbox.DayNightLength !== undefined) xml.DayNightLength = Number(sandbox.DayNightLength);
  if (sandbox.DayLightLength !== undefined) xml.DayLightLength = Number(sandbox.DayLightLength);
  if (sandbox.BloodMoonFrequency !== undefined) xml.BloodMoonFrequency = Number(sandbox.BloodMoonFrequency);
  if (sandbox.BloodMoonEnemyCount !== undefined) xml.BloodMoonEnemyCount = Number(sandbox.BloodMoonEnemyCount);
}
...
function parseGamePort(serverOrArgs) {
  if (serverOrArgs && typeof serverOrArgs === "object") {
    return clampInt(serverOrArgs.dtd?.xml?.ServerPort ?? 26900, 1024, 65535, 26900);
  }
  const match = String(serverOrArgs || "").match(/(?:^|[?&\s-])Port=(\d+)/i);
  return match ? Number(match[1]) : 26900;
}

function parseMaxPlayers(serverOrArgs) {
  if (serverOrArgs && typeof serverOrArgs === "object") {
    return clampInt(serverOrArgs.dtd?.xml?.ServerMaxPlayerCount, 1, 64, 8);
  }
...
function exeCandidates(server) {
  ...
  const dirs = [
    install,
    path.join(install, "7DaysToDie_Data", ".."),
    path.join(install, "DedicatedServer")
  ];
  const out = [];
...
function installRootFromExe(exePath) {
  return path.dirname(path.resolve(exePath));
}

async function attachToIcarusInstall(input) {
  const target = path.resolve(String(input || "").trim());
  if (!target) throw Object.assign(new Error("Choose the 7 Days to Die dedicated server folder or 7DaysToDieServer.exe"), { status: 400 });
  if (!(await pathExists(target))) throw Object.assign(new Error("That path does not exist"), { status: 404 });
  const st = await stat(target);
  if (st.isFile()) {
    const base = path.basename(target);
    if (!/7DaysToDie/i.test(base) || !/\.exe$/i.test(base)) {
      throw Object.assign(new Error("Attach to 7DaysToDieServer.exe"), { status: 400 });
    }
  ...
  if (!(await pathExists(exe))) {
    throw Object.assign(new Error("Could not find 7DaysToDieServer.exe in that folder"), { status: 400 });
  }
  return { install, exe };
}
...
function settingsIniPath(server) {
  return configXmlPath(server);
}

function configXmlPath(server) {
  const name = server.dtd?.configFile || CONFIG_FILE_NAME;
  return path.join(server.install || "", name);
}

function userDataPath(server) {
  const custom = String(server.dtd?.xml?.UserDataFolder || "").trim();
  if (custom) return custom;
  return path.join(server.install || "", "UserData");
}

const KNOWN_CONFIG_FILES = [
  "manager-serverconfig.xml",
  "serverconfig.xml",
  "serveradmin.xml",
  "startdedicated.bat"
];
...
function configFileTemplate(name) {
  ...
  if (key === "manager-serverconfig.xml" || key === "serverconfig.xml") return renderServerConfig(defaultXml());
  if (key === "serveradmin.xml") return `<?xml version="1.0"?>\n<adminTools>\n  <users>\n  </users>\n</adminTools>\n`;
  if (key === "startdedicated.bat") return "@echo off\n7DaysToDieServer -logfile logs\\output_log.txt -quit -batchmode -nographics -configfile=manager-serverconfig.xml -dedicated\n";
  return "";
}

function safeConfigFileName(name) {
  const base = path.basename(String(name || "").trim());
  if (!/^[A-Za-z0-9][A-Za-z0-9._-]{0,80}\.(xml|txt|cfg|ini|bat)$/i.test(base)) return "";
  return base;
}
...
function savedArksPath(server) {
  return userDataPath(server);
}

function defaultLaunchArgs(profile) {
  return `-logfile logs\\output_log.txt -quit -batchmode -nographics -configfile=${CONFIG_FILE_NAME} -dedicated`;
}

function applySteamServerName(launchArgs, profile) {
  return String(launchArgs || defaultLaunchArgs(profile)).trim() || defaultLaunchArgs(profile);
}
...
function applyIcarusLaunchArgs(server) {
  server.dtd = makeDtd(server.dtd);
  server.dtd.xml.ServerName = server.profile;
  let args = String(server.launchArgs || defaultLaunchArgs(server.profile)).trim() || defaultLaunchArgs(server.profile);
  if (!/-configfile=/i.test(args)) args = `${args} -configfile=${server.dtd.configFile || CONFIG_FILE_NAME}`.trim();
  else args = applyLaunchFlag(args, "-configfile", server.dtd.configFile || CONFIG_FILE_NAME);
  if (!/-dedicated(?:\s|$)/i.test(args)) args = `${args} -dedicated`.trim();
  if (!/-batchmode/i.test(args)) args = `-quit -batchmode -nographics ${args}`.trim();
  if (!/-logfile/i.test(args)) args = `-logfile logs\\output_log.txt ${args}`.trim();
...
async function writeIcarusSettings(server) {
  server.dtd = makeDtd(server.dtd);
  server.dtd.xml.ServerName = server.profile || server.dtd.xml.ServerName;
  if (!String(server.dtd.xml.UserDataFolder || "").trim() && server.install) {
    server.dtd.xml.UserDataFolder = path.join(server.install, "UserData");
  }
  const xmlPath = configXmlPath(server);
  if (!server.install) return;
  await mkdir(path.dirname(xmlPath), { recursive: true });
  await mkdir(path.join(server.install, "logs"), { recursive: true });
  await writeFile(xmlPath, renderServerConfig(server.dtd.xml), "utf8");
}

async function hydrateIcarusFromIni(server) {
  try {
    const xmlPath = configXmlPath(server);
    if (!server.install || !(await pathExists(xmlPath))) return;
    const raw = await readFile(xmlPath, "utf8");
    const parsed = parseXmlProperties(raw);
    server.dtd = makeDtd({ xml: { ...server.dtd?.xml, ...parsed }, sandbox: server.dtd?.sandbox, configFile: server.dtd?.configFile });
  } catch {
    // ignore missing/unreadable xml
  }
}
...
function publicServer(server, rcon = null) {
  ...
  const {
    ...
    logLocation, updateLogLocation, firewallStatus, firewallAutoApproved, lastBackupAt, order, steamBranch, dtd
  } = server;
  const xml = dtd?.xml || defaultXml();
  return {
    id, profile, install, exe, steamcmd, version, launchArgs,
    autostartDays, autostartTime, autostartUpdate,
    shutdownDays, shutdownTime, performUpdate, thenRestart,
...
async function getRconPublic(server) {
  const xml = makeDtd(server.dtd).xml;
  return {
    enabled: xmlBool(xml.TelnetEnabled, true),
    telnet: true,
    port: Number(xml.TelnetPort) || 8081,
    hasPassword: Boolean(String(xml.TelnetPassword || "").trim())
  };
}
...
function icarusFromIniText(raw) {
  return makeDtd({ xml: parseXmlProperties(raw) });
}

async function findSettingsIniForInstall(install) {
  const candidates = [
    path.join(install, CONFIG_FILE_NAME),
    path.join(install, "serverconfig.xml"),
    path.join(install, "myserver.xml")
  ];
...
async function resolveIcarusInstallRoot(input) {
  ...
  throw Object.assign(new Error("That folder does not look like a 7 Days to Die server (missing 7DaysToDieServer.exe)"), { status: 400 });
}
  ...
  try {
    ...
    for (const name of entries) {
      ...
      try {
        ...
        if (/-configfile=/i.test(text) || /7DaysToDieServer/i.test(text) || /ServerName/i.test(text)) {
          args += ` ${text}`;
        }
...
async function inspectIcarusInstall(sourcePath, { measure = true } = {}) {
  ...
  const dtd = icarusFromIniText(raw);
  const profile = String(
    dtd.xml.ServerName || path.basename(install) || "Imported Server"
  ).replace(/"/g, "").trim() || "Imported Server";
  const launchArgs = defaultLaunchArgs(profile);
...
async function runImportJob(job) {
  ...
  try {
    ...
    const server = makeServer({
      ...
      dtd: preview.dtd || preview.icarus,
      order: state.servers.length
    });
    server.dtd = makeDtd(preview.dtd || preview.icarus);
...
function shooterLogPath(server) {
  const install = server.install || "";
  return path.join(install, "logs", "output_log.txt");
}
...
async function getArkVersionFromLogs(install) {
  const logsDir = path.join(install, "logs");
  ...
  const pattern = /(?:GameVers|Version|Build)[:\s]+([\d.]+[A-Za-z0-9._-]*)/i;
...
async function resolveProspectFile() {
  return "";
}
...
async function steamPersonaName(steamId) {
  ...
  try {
    const res = await fetch(`https://steamcommunity.com/profiles/${id}/?xml=1`, {
      headers: { "User-Agent": "SevenDaysToDieServerManager/1.0" },
...
async function playersFromLogs(server) {
  try {
    const exclude = [server?.dtd?.xml?.GameName, server?.profile];
...
async function detectReadyFromLogs(install) {
  const logPath = shooterLogPath({ install });
  const text = await readLogTail(logPath);
  if (!text) return false;
  return /INF StartGame|Game started|Dedicated server|Awake done|Started server/i.test(text);
}
...
async function refreshRuntime(server, { deep = false, procs = null } = {}) {
  ...
  if (match) {
    ...
    const queryPort = parseGamePort(server);
...
async function ensureManagerFirewallPort(port) {
  if (process.platform !== "win32") return;
  const ruleName = `7 Days To Die Server Manager TCP ${port}`;
...
async function ensureFirewall(server) {
  ...
  const xml = makeDtd(server.dtd).xml;
  const mainPort = clampInt(xml.ServerPort, 1024, 65535, 26900);
  const telnetPort = clampInt(xml.TelnetPort, 1, 65535, 8081);
  const dashPort = clampInt(xml.WebDashboardPort, 1, 65535, 8080);
  const ports = [
    { port: mainPort, protocol: "UDP" },
    { port: mainPort, protocol: "TCP" },
    { port: mainPort + 1, protocol: "UDP" },
    { port: mainPort + 2, protocol: "UDP" },
    { port: telnetPort, protocol: "TCP" }
  ];
  if (xmlBool(xml.WebDashboardEnabled, false)) {
    ports.push({ port: dashPort, protocol: "TCP" });
  }

  const rules = ports.map(item => ({
    name: `7DTD Server: ${server.profile} ${item.protocol} ${item.port}`,
...
async function copyServerLogOnStop(server) {
  ...
  const src = shooterLogPath(server);
...
async function startServer(server, { applyFirewall = false } = {}) {
  ...
  if (!(await pathExists(exe))) {
    throw Object.assign(new Error(`7DaysToDieServer.exe was not found in:\n${server.install}`), { status: 404 });
  }
  server.exe = exe;
...
async function stopServer(server, { copyLog = true } = {}) {
  ...
  if (!gone) {
    appendConsoleLog(server.id, "Stop requested, but 7DaysToDieServer.exe is still running.", "error");
  } else {
    await delay(1500);
  }
...
async function wipeInstallKeepSaved(server) {
  ...
  appendConsoleLog(server.id, "Repair mode: wiping server files but keeping UserData, Mods, and manager-serverconfig.xml…", "system");
  const entries = await readdir(install, { withFileTypes: true });
  const keep = new Set(["userdata", "mods", "logs", CONFIG_FILE_NAME.toLowerCase(), "serveradmin.xml"]);
  for (const entry of entries) {
    const full = path.join(install, entry.name);
    if (keep.has(entry.name.toLowerCase())) {
      appendConsoleLog(server.id, `Keeping ${entry.name}`, "system");
      continue;
    }
...
function spawnSteamCmdUpdate(server, steamcmdExe, { validate = true, timeoutMs = 90 * 60 * 1000 } = {}) {
  return new Promise(async (resolve) => {
    const branch = String(server.steamBranch || "").trim();
    const args = [
...
async function runSteamUpdate(server, {
  ...
  try {
    ...
    if (result.timedOut) {
      ...
      appendConsoleLog(
        server.id,
        "SteamCMD 0x6 persisted. Use Repair & Redownload to wipe the install except UserData/Mods/config, then download fresh.",
        "error"
      );
      ...
      if (await pathExists(exe)) {
        ...
        appendConsoleLog(server.id, "Update finished, but 7DaysToDieServer.exe was not found in the install folder.", "error");
      }
...
async function ensureModsFolder(server) {
  if (!server.install) throw Object.assign(new Error("Install location is not set"), { status: 400 });
  const mods = path.join(server.install, "Mods");
...
async function handleApi(req, res, url) {
  ...
  if (!ALLOW_REMOTE && !isLoopbackRequest(req)) {
    return sendJson(res, 403, { error: "Remote access is disabled. Set SEVENDTD_ALLOW_REMOTE=true to enable." });
  }
  if (ALLOW_REMOTE && !ALLOW_PUBLIC && !isLoopbackRequest(req) && !isPrivateLanAddress(remote)) {
    return sendJson(res, 403, { error: "Only loopback/LAN clients are allowed. Set SEVENDTD_ALLOW_PUBLIC=true to allow all IPs." });
  }
  ...
  if (method === "POST" && pathname === "/api/manager/restart") {
    ...
    if (!(await pathExists(script))) {
      return sendJson(res, 500, { error: "StartSevenDaysManager.ps1 was not found next to server.mjs" });
    }
    if (!(await pathExists(helperCmd))) {
      return sendJson(res, 500, { error: "RestartSevenDaysManager.cmd was not found next to server.mjs" });
    }
  ...
  if (method === "POST" && pathname === "/api/path/browse") {
    ...
    const body = (await readBody(req)) || {};
    const result = await browseFolderDialog(body.title || "Select 7 Days to Die server folder");
  ...
  if (method === "PATCH" && !action) {
    ...
    if (body.profile !== undefined) {
      server.dtd = makeDtd({ ...server.dtd, xml: { ...server.dtd.xml, ServerName: server.profile } });
      server.launchArgs = applyIcarusLaunchArgs(server);
    }
    if (body.dtd && typeof body.dtd === "object") {
      server.dtd = makeDtd({
        ...server.dtd,
        ...body.dtd,
        xml: { ...server.dtd.xml, ...(body.dtd.xml || {}) },
        sandbox: { ...server.dtd.sandbox, ...(body.dtd.sandbox || {}) }
      });
      server.launchArgs = applyIcarusLaunchArgs(server);
      if (server.install) {
    ...
    if (body.xml && typeof body.xml === "object") {
      server.dtd = makeDtd({ ...server.dtd, xml: { ...server.dtd.xml, ...body.xml } });
      server.launchArgs = applyIcarusLaunchArgs(server);
      if (server.install) {
        try { await writeIcarusSettings(server); } catch { /* ignore */ }
      }
    }
    if (body.sandbox && typeof body.sandbox === "object") {
      server.dtd = makeDtd({ ...server.dtd, sandbox: { ...server.dtd.sandbox, ...body.sandbox } });
      if (server.install) {
        try { await writeIcarusSettings(server); } catch { /* ignore */ }
      }
    }
  ...
  if (method === "POST" && action === "command") {
    ...
    if (asChat) command = `say "${command.replace(/"/g, "")}"`;
    ...
    try {
      const reply = await telnetExec(server, command);
      if (reply) appendConsoleLog(server.id, reply, "log");
      return sendJson(res, 200, { ok: true, reply });
    } catch (err) {
      appendConsoleLog(server.id, err.message, "error");
      return sendJson(res, 200, { ok: false, reply: err.message });
    }
  }
...
function telnetExec(server, command, timeoutMs = 8000) {
  const xml = makeDtd(server.dtd).xml;
  const port = clampInt(xml.TelnetPort, 1, 65535, 8081);
  const password = String(xml.TelnetPassword || "");
  return new Promise((resolve, reject) => {
    ...
    let sent = false;
    let settled = false;
    const timer = setTimeout(() => finish(new Error("Telnet timed out")), timeoutMs);
    const finish = (err, value) => {
      if (settled) return;
      settled = true;
      clearTimeout(timer);
    ...
    socket.on("data", chunk => {
      ...
      if (!authed && /password/i.test(lower)) {
        socket.write(`${password}\n`);
        authed = true;
        buffer = "";
        return;
      }
      if (authed && !sent && /session|logon|control|help|>/i.test(lower)) {
    ...
    socket.on("connect", () => {
      setTimeout(() => {
        if (!sent) {
          if (password && !authed) socket.write(`${password}\n`);
          socket.write(`${command}\n`);
          sent = true;
        }
      }, 400);
    ...
    socket.on("error", err => finish(err));
    socket.on("close", () => {
      if (!settled) finish(null, buffer.replace(/\u0000/g, "").trim());
    });
  });
}
...
async function main() {
  ...
  server.on("error", err => {
    if (err && err.code === "EADDRINUSE") {
      console.error(`Port ${PORT} is already in use. Close the other 7 Days To Die Server Manager window, or run Start 7DTD Manager.cmd again.`);
      process.exit(1);
    }
    console.error(err);
    process.exit(1);
  ...
  server.listen(PORT, HOST, () => {
    ...
    console.log(`7 Days To Die Server Manager listening on ${HOST}:${PORT}`);
