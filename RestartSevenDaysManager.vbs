' Fully detaches RestartSevenDaysManager.cmd so it survives when the node process exits.
Option Explicit
Dim sh, fso, scriptDir, helper, port, host, cmd, logFile, ts
Set sh = CreateObject("WScript.Shell")
Set fso = CreateObject("Scripting.FileSystemObject")
scriptDir = fso.GetParentFolderName(WScript.ScriptFullName)
helper = scriptDir & "\RestartSevenDaysManager.cmd"
If WScript.Arguments.Count >= 1 Then port = WScript.Arguments(0) Else port = "3240"
