...
$env:SEVENDTD_PORT = "$Port"
$env:SEVENDTD_HOST = $HostAddress
if (-not $env:SEVENDTD_ALLOW_REMOTE) { $env:SEVENDTD_ALLOW_REMOTE = "true" }
if (-not $env:SEVENDTD_ALLOW_PUBLIC) { $env:SEVENDTD_ALLOW_PUBLIC = "true" }

$PublicRepoUrl = "https://github.com/Genis221/7-Days-To-Die-Server-Manager.git"
...
function Ensure-Git {
  ...
  Write-Host "Git installed but is not on PATH yet. Close this window and run Start 7DTD Manager.cmd again." -ForegroundColor Yellow
  return $false
}
...
function Ensure-Node {
  ...
  if ($node) {
    try {
      ...
      Write-Host "Node.js $major detected; 7DTD Manager needs Node.js 20+." -ForegroundColor Yellow
    } catch { }
  } else {
  ...
  if (-not $node) {
    Write-Host "Node.js installed but is not on PATH yet. Close this window and run Start 7DTD Manager.cmd again." -ForegroundColor Yellow
    return $false
  }
  try {
    $major = [int]((& node -p "process.versions.node.split('.')[0]").Trim())
...
function Update-7DTDManagerFromGit {
  if (-not (Ensure-Git)) {
  ...
  Push-Location $projectRoot
  try {
    if (-not (Ensure-7DTDGitRepo)) { return $false }

    Write-Host "Checking GitHub for 7DTD Manager updates..." -ForegroundColor DarkGray
    ...
    Write-Host "Updating 7DTD Manager from $remoteRef..." -ForegroundColor Yellow
...
function Ensure-ManagerFirewallPort {
  ...
  $ruleName = "7 Days To Die Server Manager TCP $Port"
  ...
  if ($LASTEXITCODE -eq 0 -and $show -notmatch "No rules match") {
    Write-Host "Firewall already allows 7DTD Manager port $Port (TCP)." -ForegroundColor DarkGray
    return
  }

  Write-Host "Opening Windows Firewall for 7DTD Manager port $Port (TCP)..." -ForegroundColor Cyan
...
$lanIp = Get-LanIPv4
Write-Host "Starting 7 Days To Die Server Manager on port $Port..." -ForegroundColor Cyan
...
Write-Host "  WAN:     forward TCP $Port to this PC for the panel (7DTD also needs UDP game + query ports)"
